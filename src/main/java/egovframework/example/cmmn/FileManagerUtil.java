...
public final class FileManagerUtil {
	...
	public static void main(String[] args) {
		try {
			...
			// 초기화 chunkFile,chunkFile.END chunkFile으로 시작되는 모든 파일을 삭제
			String filePath = "/home/john/devHome/file/create/";
			String fileName = "chunkFile";
			String endfileName = "chunkFile.END";
			...
			if (isFIle) {
				...
				if (file.exists()) {
					...
					file = new File(filePath + endfileName);
					file.createNewFile();
	...
	public static int deleteAllFilesByName(String dirPath, String fileName) throws IOException {
		...
		try (Stream<Path> list = Files.list(directory)) {
			List<Path> targets = list.filter(Files::isRegularFile)
					.filter(p -> p.getFileName().toString().startsWith(fileName)).collect(Collectors.toList());
